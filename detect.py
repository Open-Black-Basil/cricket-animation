...
def run(
    ...
    def calculate_iou(box1, box2):
        ...
        Args:
            box1 (Tensor): Tensor representing the first bounding box in [x1, y1, x2, y2] format.
            box2 (Tensor): Tensor representing the second bounding box in [x1, y1, x2, y2] format.
    ...
    # def draw_lines(image, points, color=(0, 150, 255), thickness=2):
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if iou > 0.25:
                        ...
                        font_scale = 1
                        font_color = (255,255, 0)  # BGR format
                        thickness = 2

                        # Determine the text size
                        text_size, _ = cv2.getTextSize(text, font, font_scale, thickness)

                        # Calculate the position to write the text (centered)
                        # text_x = int((im0.shape[1] - text_size[0]) / 2)
                        text_x = im0.shape[1] - 10 - text_size[0]
                        text_y = int((im0.shape[0] + text_size[1]) / 2)

                        # Write the text on the image
                        
                        cv2.putText(im0, text, (text_x, text_y), font, font_scale, font_color, thickness)
                    # if ball_detection:
                    #     cv2.circle(im0, (200, 150), 5, (255,0,0), 5)
                    ...
                    # if len(ball_detection) > 1:
                    #     print(f"ball detect - {ball_detection}")
                    #     # center = tuple((ball_detection[0]))
                    #     # print(f"ball detect tuple - {center}")
                    #     # im0 = cv2.circle(im0,center, 20, (255,150,0), 8)
                    #     for i in range(len(points) - 1):
                    #     pt1 = tuple(points[i])
                    #     pt2 = tuple(points[i + 1])
                    #     cv2.line(image, pt1, pt2, color, thickness)
