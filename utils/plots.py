...
class Annotator:
    ...
    def convert_to_cartoon(self,image):
        ...
        # Apply bilateral filter to obtain a cartoon-like effect
    ...
    def box_label(self, box, label='', balltrek=False, cartoon=False, color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if label.startswith("ball") and balltrek:
            ...
            lenghtOfBallDetection = len(ball_detection)
            if lenghtOfBallDetection > 1:
                for i in range(lenghtOfBallDetection - 1):
                    pt1 = tuple(ball_detection[i])
                    pt2 = tuple(ball_detection[i + 1])
        ...
        if self.pil or not is_ascii(label):
            ...
            # print(f" self box -=={box} and width= {self.lw} and p1 ={p1} and p2= {p2}")
            cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            '''code to draw circle
            center_x = (int(box[0]) + int(box[2])) // 2
            center_y = (int(box[1]) + int(box[3])) // 2
            cv2.circle(self.im,(center_x,center_y), 20, (255,150,0), 8) '''
            if label:
